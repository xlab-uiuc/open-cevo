...
BASE_URL = "https://github.com/apache/hadoop/commit/"

#RE for config File for HDFS
HDFS_CONFIG_FILE_RE = '[a-zA-Z\.\_\-]*-default.xml'

#RE for config Load in HDFS
HDFS_CONFIG_LOAD_FUNC_RE = '[a-zA-Z\.\_\-]*[cC]onf[ig]*[\(\)]*[\s]*[\n]*[\s]*\.[\s]*[\n]*[\s]*get[a-zA-Z]*\([^;<>]+\)'

#RE for config assign in HDFS
HDFS_CONFIG_ASSIGN_RE = '[a-zA-Z\.\_\-]+[\s]*=[\s]*[\n]*[\s]*' + HDFS_CONFIG_LOAD_FUNC_RE

#RE for config set in HDFS
HDFS_CONFIG_SET_FUNC_RE = '[a-zA-Z\.\_\-]*[cC]onf[ig]*[\(\)]*[\s]*[\n]*[\s]*\.[\s]*[\n]*[\s]*set[a-zA-Z]*\([^;<>]+\)'

#RE for system parameter load in HDFS
HDFS_SYS_PARAM_LOAD_FUNC_RE = 'System\.get(?:Property|env)\([^)^;]+\)'

#RE for system parameter assign in HDFS
HDFS_SYS_PARAM_ASSIGN_FUNC_RE = '[a-zA-Z\.\_\-]+[\s]*=[\s]*[\n]*[\s]*' + HDFS_SYS_PARAM_LOAD_FUNC_RE

#RE for system parameter set in HDFS
HDFS_SYS_PARAM_SET_FUNC_RE = 'System\.set(?:Property|env)\([^)^;]+\)'
...
def diffSelection(url,config_variable_list):
    ...
    #the set of touched configuration Variableeter
    touchedVariable = []
    ...
    if codeSet and diffSet:
        ...
        for codeElement in codeSet:
            configAssignObj = re.findall(HDFS_CONFIG_ASSIGN_RE,codeElement.code_snippet,re.M | re.I)
            if configAssignObj:
                for assignObj in configAssignObj:
                    collect_config_variable(assignObj,codeElement,config_variable_list)

            sysParamAssignObj = re.findall(HDFS_SYS_PARAM_ASSIGN_FUNC_RE,codeElement.code_snippet,re.M | re.I)
        ...
        for diffElement in diffSet:
            ...
            configFileObj = re.findall(HDFS_CONFIG_FILE_RE,diffElement.diff_class,re.M | re.I)
            if configFileObj:
            ...
            configLoadObj = re.findall(HDFS_CONFIG_LOAD_FUNC_RE,diffElement.diff_snippet,re.M | re.I)
            if configLoadObj:
            ...
            sysParamLoadObj = re.findall(HDFS_SYS_PARAM_LOAD_FUNC_RE,diffElement.diff_snippet,re.M | re.I)
            if sysParamLoadObj:
            ...
            configSetObj = re.findall(HDFS_CONFIG_SET_FUNC_RE,diffElement.diff_snippet,re.M | re.I)
            if configSetObj:
            ...
            sysParamSetObj = re.findall(HDFS_SYS_PARAM_SET_FUNC_RE,diffElement.diff_snippet,re.M | re.I)
            if sysParamSetObj:
